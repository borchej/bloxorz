...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filterNot(b => explored.contains(b._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case _ =>
      val neighbours = (for {
        b <- initial
      } yield newNeighborsOnly(neighborsWithHistory(b._1, b._2), explored)).flatten
      initial #::: from(neighbours, explored ++ neighbours.map(_._1).toSet)
  }
  ...
  lazy val solution: List[Move] =
    if(pathsToGoal.size > 0) pathsToGoal(0)._2.reverse
    else List.empty
}
